...
// Mở khóa audio khi người dùng click 1 lần
...
function seqSmallBarrage() {
  ...
  const barrageCount = IS_DESKTOP ? 20 : 10;
  ...
  while (count < barrageCount) {
    if (count === 0) {
      ...
      const delayOffset = Math.random() * 10 + 10;
...
// Biến kiểm soát trạng thái countdown
...
function updateGlobals(timeStep, lag) {
  ...
  // auto launch shells chỉ khi countdown đã kết thúc
  if (store.state.config.autoLaunch && !window.isCountdownRunning) {
